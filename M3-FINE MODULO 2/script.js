//Premessa: ho voluto creare un array per simulare un ambiente dove gli elementi sono aggiunti aggiornando gli oggetti in esso contenuti.
// L'array è stato generato inizialmente da me per poi essere inserito in chatGpt per aggiornare le chiavi valori e creare la voce Visto e Numero di visualizzazioni.
// Non ho avuto il tempo di poter affinare il progetto ma con le chiavi valore inserite posso creare le categorie principali solitamente visualizzate nei siti e app di streaming
...
const list = [
	{
		...
		descrizione: "Sto Imparando ad inserire l'API summary da Wikipedia per non scrivere la descrizione manualmente",
		genere: "thriller",
		Visto: true,
...
function CreaOggettiArray(Titolo, alt, descrizione) {
	...
	const Visto = false;
	const NumeroDiVisualizzazioni = Math.floor(Math.random() * (1000 - 10 + 1)) + 10;

	const NuovoTitolo = {
		Titolo: titolo,
		img: imgPath,
		Alt: alt,
		descrizione: descrizione || "Sto Imparando ad inserire l'API summary da Wikipedia per non scrivere la descrizione manualmente",
		genere: genere,
		Visto: Visto,
...
for (let i = list.length; i < 25; i++) {
	CreaOggettiArray("Titolo " + (i + 1), "alt" + (i + 1), "Descrizione serie " + (i + 1));
}
...
const sezioni = ["Trending Now", "Watch It Again", "New Releases", "Thriller", "Drama", "Commedy", "Movies", "Sci-Fi", "History", "Documentary", "Horror", "New Series", "Original Netflix", "Halloween"];

function shuffleArray(array) {
	for (let i = array.length - 1; i > 0; i--) {
		const j = Math.floor(Math.random() * (i + 1));
		[array[i], array[j]] = [array[j], array[i]]; //ricordare!!!
	}
	return array;
}
...
// Crea 5 righe, modifica i per ottenere infinite righe, simulo un ambiente dove i titoli sono inseriti dentro un array ed è sufficente implementare un Oggetto al suo interno per aggiornare la lista
for (let i = 0; i < 5; i++) {
	rowCreator();
}
